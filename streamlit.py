...
# 定义一个函数，使用用户密钥对 API 进行身份验证、发送提示，并获取AI生成的响应。该函数接受用户的提示作为参数，并使用st.info在蓝色框中显示AI生成的响应
...
def get_vectordb():
    ...
    # 向量数据库持久化路径
    persist_directory = 'data_base/vector_db/chro'
    # 加载数据库
    vectordb = Chroma(
...
def get_chat_qa_chain(question:str,zhipuai_api_key:str):
    ...
    llm = ZhipuAILLM(model="chatglm_std", temperature = 0,zhipuai_api_key = zhipuai_api_key)
